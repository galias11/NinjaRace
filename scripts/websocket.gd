...
const USER_AGENT = "Ninja-Race/1.0"
...
func _run(_self):
	...
	while is_connected_to_host():
		...
		while messages.size()>0:
			...
			messages.pop_front()
			var aux = ""
			for i in range(ceil((msg.length() / 100) + 0.5)):
				aux = msg.substr(i * 100, 100)
				byte = 0x00
				if i == 0:
					byte = 0x01
				if i + 1 == ceil((msg.length() / 100) + 0.5):
					byte = 0x80
				if ceil((msg.length() / 100) + 0.5) == 1:
					byte = 0x81
				put_u8(byte)
				byte = 0x00 | aux.length() + 4
				put_u8(byte)
				put_utf8_string(aux)
			
			#print("mando: ", msg.payload.state)
			
			# mount frame
			#byte = 0x80 # fin
			#byte = byte | 0x01 # text frame
			#put_8(byte)
			#byte = 0x00 | (msg.length() + 4) # mask flag and payload size
			#put_u8(byte)
			#put_utf8_string(msg)
...
func start(host,port,path=null):
	...
	if OK==connect_to_host(host, port):
